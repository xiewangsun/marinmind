...
| 模块 | 说明 | 状态 |
| ---- | ---- | ---- |
| 阅读与标注 | PDF 阅读；文字/语音/照片/手写等多形式批注；高亮、矩形/折线摘录 | 🚧 PDF 阅读 + 区域框选/文字划选摘录已实现 |
| 摘录→卡片 | 文档中的任何摘录（文字、区域、手写、语音）自动变成一张「知识卡片」 | ✅ 区域摘录与文字摘录闭环 |
| 思维导图 | 拖拽卡片构建思维导图，卡片与原文位置双向关联；支持卡片合并、链接 | 🚧 数据层就绪 |
| 学习卡（闪卡） | 摘录卡片直接转为闪卡，内置间隔重复复习 | ✅ 复习界面闭环（转闪卡/翻面/四档评分/跳转原文） |
| 多窗格工作区 | 阅读/脑图/卡片同屏多窗格切换（学习模式 / 研究模式） | ⬜ |
| OCR | 支持扫描版 PDF 文字识别 | ⬜ |
| 混合文档 | 多本书籍放进同一笔记本，跨书做一张脑图 | ✅ 数据层支持 |
