...
/** 宿主注入项：节点快照提供者 + 次行描述 + 选中回调 */
export interface NodeSearchModalOptions {
	/** 每次搜索现拉（打开期间跨标签改图也能看到最新） */
	nodes: () => readonly MindmapNodeWithCard[];
	/** 结果次行描述（《书名》· 第 N 页 / 手工卡片，宿主注入避免依赖 plugin 类型） */
	describe: (node: MindmapNodeWithCard) => string;
 ...
 * （主行 = 命中字段全文截断，徽标 = 标题/批注/摘录，次行 = 书名·页码）+
 * ↑↓/Enter 键盘导航 + 底部计数（截断时"前 100"）。匹配逻辑在 node-search.ts。
 */
export class NodeSearchModal extends Modal {
	...
	private renderList(): void {
		...
		this.rows.forEach((row, i) => {
			...
			name.createSpan({ cls: "marinmind-picker-title", text: row.main });
			// 次行：书名 · 页码（宿主注入）
			item.createDiv({
				cls: "marinmind-picker-dir",
				text: this.opts.describe(row.node),
			});
	...
	private moveSelection(delta: number): void {
		...
		this.selected =
			(this.selected + delta + this.rows.length) % this.rows.length; // 环绕
		this.refreshSelection();
		this.listEl
