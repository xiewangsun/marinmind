...
export interface Card {
	...
	title: string | null;
	/**
	 * 闪卡遮挡区域（㊷）：页归一化矩形列表，复习正面遮住（可揭开）、翻面自动显示；
	 * 阅读器只画虚线标记不遮内容。空数组序列化省略（零写入契约）。
	 * 仅支持有页矩形的形态（text/area/lasso/handwriting/blank），photo/audio 无此功能
	 */
	occlusions: DocRect[];
	tags: string[];
	createdAt: number;
...
/**
 * 脑图分支样式（参照 MarginNote 4 分支样式，⑱）：
 * tree=树形（右，默认）/ tree-left=树形（左，镜像）/ tree-down=树形（下，组织架构图）/
 * line=直线（横向链条）/ bidir=双向（根居中左右对称）/ frame=框架（收纳框代替连线）。
